...
```{r}
#get the group mean differences and their standard deviation.
...
```{r}
#Plot the mean profile to check if there is interaction between two independent variables.
library(ggplot2)
...
```{r}
#model fitting using two way anova
...
library(gridExtra)
library(ggplot2)
#Get predicted value, and residuals from anova model fitting and assign to a dataframe called myfits.
...
```
```{r}
#If we are only interested in the comparison between groups of Gtl:Y-Gtl:N", "Mod:Y-Mod:N" and "Sev:Y-Sev:N".
library(lsmeans)
...
final.result
```
>>>>>>> add two way anova to project rmd file and write the part of report
